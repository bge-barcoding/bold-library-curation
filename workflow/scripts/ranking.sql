...
SELECT
    ...
    CASE
        ...
        WHEN pc.SPECIES_ID = 1 AND pc.SEQ_QUALITY = 1 AND pc.HAS_IMAGE = 1 AND pc.COLLECTORS = 1 AND pc.COLLECTION_DATE = 1 AND pc.COUNTRY = 1 AND (pc.SITE = 1 OR pc.SECTOR = 1 OR pc.REGION = 1 OR pc.COORD = 1) AND pc.IDENTIFIER = 1 AND (pc.ID_METHOD >= 1 OR pc.INSTITUTION = 1) AND (pc.PUBLIC_VOUCHER = 1 OR pc.MUSEUM_ID = 1) THEN 2
